...
func GenerateInitConfig(opts InitOptions) (*InitResult, error) {
	if opts.Suggest && opts.Preset != "" && opts.Preset != "auto" {
		return nil, fmt.Errorf("--suggest cannot be combined with preset %q", opts.Preset)
	}
	...
	configYAML := renderConfigWithMode(ignorePaths, includePathStrings, layers, opts.Strict, opts.Suggest)
...
func renderConfig(ignorePaths []string, includePaths []string, layers []discoveredLayer, strict bool) string {
	return renderConfigWithMode(ignorePaths, includePaths, layers, strict, false)
}

func renderConfigWithMode(ignorePaths []string, includePaths []string, layers []discoveredLayer, strict bool, suggest bool) string {
	var b bytes.Buffer

	b.WriteString("# Generated by PatchCourt.\n")
	b.WriteString("# Review this file before using it in CI.\n")
	if suggest {
		b.WriteString("# Suggested config: layers are inferred from the current project structure.\n")
		b.WriteString("# Use this to replace outdated or too narrow .patchcourt.yaml files.\n")
	} else {
